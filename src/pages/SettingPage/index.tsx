...
const SettingsPage = () => {
  ...
  const { mutate: updateUser, isPending: isUpdatingUser } = useMutation({
    mutationFn: () => updateById("authen/api/v1/users", editUser!.id, editForm),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users"] });
      setEditUserOpen(false);
  ...
  const { mutate: addRoom, isPending } = useMutation({
    mutationFn: async () => {
      ...
      try {
        const room = await postBody("room/api/v1/room", payload);
        return room;
      } catch (err) {
    ...
    onError: (err: any) => {
      Swal.fire({
        icon: "error",
        title: "เกิดข้อผิดพลาด",
        text: err?.response?.data?.detail || "ไม่สามารถเพิ่มห้องเรียนได้",
        confirmButtonColor: "#ef4444",
      });
    },
  });
              ...
              <Box className="bg-white rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] w-full max-w-3xl mx-4 sm:mx-6 overflow-hidden">
                ...
                <Box className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
                  <Box className="flex items-center gap-3">
                    ...
                    <p className="text-lg font-semibold">เพิ่มห้องเรียนใหม่</p>
                  </Box>
                  <button
                ...
                <Box className="px-5 py-4">
                  <Box className="grid sm:grid-cols-2 md:grid-cols-2 gap-4">
                    <Box className="flex flex-col gap-1.5">
                      <label className="text-sm text-gray-600">ชื่อห้อง</label>
                      <input
                        type="text"
                        value={newRoom.name}
                        onChange={(e) =>
                    ...
                    <Box className="flex flex-col gap-1.5">
                      <label className="text-sm text-gray-600">ครูประจำ</label>
                        <Select
                            isMulti
                            placeholder="เลือกครู..."
                            className="rounded-xl"
                            classNamePrefix="select"
                            options={users.filter((u) => u.role === "teacher" && u.is_active)
                                .map((t) => ({
                                    value: t.id,
                            ...
                            value={users
                                ...
                                .filter((opt) => newRoom.teacher.includes(opt.value))}
                            onChange={(selected) =>
                                setNewRoom({
                                ...newRoom,
                                teacher: (selected ?? []).map((s) => s.value),
                                })
                            }
                        />
                      {/* <Select
                        ...
                        onChange={(e) => {
                            ...
                            setNewRoom({ ...newRoom, teacher: selectedValues });
                        }}
                      ...
                      >
                        <option value="">เลือกครูประจำ</option>
                        {users
                          .filter((u) => u.role === "teacher" && u.is_active)
                          .map((t) => (
               ...
               file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer"
                ...
                <Box className="flex justify-end gap-3 px-5 py-4 border-t border-gray-200">
                  ...
                  >
                    ...
                    {isPending ? "กำลังบันทึก..." : "เพิ่มห้องเรียน"}
                  </button>
                </Box>
              </Box>
            </Box>
              ...
              <Box className="bg-white rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] w-full max-w-2xl overflow-hidden">
                ...
                <Box className="flex justify-end gap-3 px-5 py-4 border-t border-gray-200">
                  ...
                  >
                    ...
                    {isAddUserPending ? "กำลังบันทึก..." : "เพิ่มผู้ใช้"}
                  </button>
                </Box>
              </Box>
            </Box>
                  ...
                  <Box className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl overflow-hidden">
                    ...
                    <Box className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
                      <Box className="flex items-center gap-2">
                        ...
                        <p className="font-semibold text-lg">แก้ไขห้องเรียน</p>
                      </Box>
                      <button
                        onClick={() => {
                      ...
                      <Box className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <Box className="flex flex-col gap-1.5">
                          ...
                          <Select
                            ...
                            value={users
                              .filter((u) =>
                                editRoom?.staff_ids?.includes(u.id)
                              )
                              .map((t) => ({
                                value: t.id,
                                label: `${t.first_name} ${t.last_name}`,
              ...
              <Box className="relative z-10 w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
                <Box className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
                  <Box className="flex justify-center gap-3">
                    ...
                    <p className="text-lg font-semibold">แก้ไขผู้ใช้</p>
                  </Box>
                  <button
                    onClick={closeEdit}
              ...
              <Box className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4 mt-8">
                <Box className="bg-blue-50 border border-blue-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-blue-700 text-sm">ผู้ใช้ทั้งหมด</p>
                  </Box>
                </Box>
                <Box className="bg-emerald-50 border border-emerald-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-emerald-700 text-sm">ใช้งานได้</p>
                  </Box>
                </Box>
                <Box className="bg-purple-50 border border-purple-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-purple-700 text-sm">ครู</p>
                  </Box>
                </Box>
                <Box className="bg-red-50 border border-red-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-red-700 text-sm">ผู้ดูแลระบบ</p>
                  </Box>
                </Box>
              </Box>
            </Box>
                      ...
                      <Box className="flex flex-col gap-3">
                        <Box className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl border border-gray-200">
                          <Box className="flex items-center gap-2">
                            ...
                            <p className="text-gray-600">จำนวนเด็ก</p>
                          </Box>
                        ...
                        <Box className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl border border-gray-200">
                          <Box className="flex items-center gap-2">
                            ...
                            <p className="text-gray-600">ช่วงอายุ</p>
                          </Box>
                          <p className="font-semibold">
                            {formatAgeRange(room.min_age, room.max_age)}
                          </p>
                        </Box>
                        ...
                        <Box className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl border border-gray-200">
                          <Box className="flex items-center gap-2">
                            ...
                            <p className="text-gray-600">ครูประจำ</p>
                          </Box>
                          <p className="font-semibold">
                            {room.teachers.map((t) => t.staff).join(", ")}
                          </p>
                        </Box>
                      </Box>
                    </Box>
              ...
              <Box className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4 mt-8">
                <Box className="bg-emerald-50 border border-emerald-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-emerald-700 text-sm">ห้องเรียนทั้งหมด</p>
                  </Box>
                </Box>
                <Box className="bg-blue-50 border border-blue-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-blue-700 text-sm">เด็กทั้งหมด</p>
                  </Box>
                </Box>
                {/* Average */}
                <Box className="bg-purple-50 border border-purple-200 rounded-2xl">
                  <Box className="text-center p-6">
                    ...
                    <p className="text-purple-700 text-sm">เฉลี่ยต่อห้อง</p>
                  </Box>
                </Box>
              </Box>
            </Box>
            ...
            <Box className="p-6">
              ...
              <Box className="flex items-center justify-between mb-8">
                <Box>
                  ...
                  <p className="text-[#6B7280] text-lg">เพิ่ม และ แก้ไข</p>
                </Box>
                <button
                    ...
                    <Box className="p-3 sm:p-4">
                      ...
                      <Box className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <Box className="flex-1 min-w-0">
                            ...
                            <Box className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mt-2">
                              ...
                              <Box className="flex items-center gap-2 min-w-0">
                                ...
                                />
                                <p className="truncate">
                                  ประเมินล่าสุด: {student.lastEvaluationDate}
                                </p>
                              </Box>
                            </Box>
                          </Box>
                        ...
                        <Box className="flex flex-col sm:flex-row gap-2 sm:items-center w-full sm:w-auto">
                          ...
                          <button
                            onClick={
                              () => handleDeleteStudent(student.id) // handleDelete สำหรับการลบข้อมูลของ student
                            }
